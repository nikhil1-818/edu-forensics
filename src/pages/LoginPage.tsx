...
export const LoginPage: React.FC<LoginPageProps> = ({ navigate }) => {
  ...
  // Google Sign-In state: Anyone can login with their own personal Gmail or Google Workspace account
  const [googleEmail, setGoogleEmail] = useState(() => {
    try {
      return localStorage.getItem('eduforensics_saved_google_email') || '';
    } catch {
      return '';
    }
  });
  const [googleName, setGoogleName] = useState(() => {
    try {
      return localStorage.getItem('eduforensics_saved_google_name') || '';
    } catch {
      return '';
    }
  });
  ...
  // Google Login Handler: supports ANY Google account entered by the user
  const handleGoogleLogin = async (e?: React.FormEvent) => {
    ...
    try {
      const emailToUse = (googleEmail.trim() || 'academic.user@gmail.com').toLowerCase();
      const nameToUse = googleName.trim() || emailToUse.split('@')[0].replace(/[._]/g, ' ').replace(/\b\w/g, (l: string) => l.toUpperCase());

      // Remember the user's Google account for their future logins on this browser
      try {
        localStorage.setItem('eduforensics_saved_google_email', emailToUse);
        localStorage.setItem('eduforensics_saved_google_name', nameToUse);
      } catch {
        // ignore
      }
    ...
    <div className="min-h-screen bg-slate-50 flex">
      ...
      <div className="w-full lg:w-1/2 flex items-center justify-center p-5 sm:p-8 md:p-12 overflow-y-auto">
        <div className="w-full max-w-xl my-auto">
          ...
          {/* ==================================================== */}
          {/* TAB 1: GOOGLE / GMAIL ACCOUNT SIGN IN (UNIVERSAL) */}
          {/* ==================================================== */}
          {activeTab === 'google' && (
            <div className="bg-white rounded-2xl border border-slate-200 p-5 sm:p-6 shadow-xs animate-in fade-in">
              <div className="flex items-start justify-between mb-4">
                <div>
                  <h3 className="text-sm font-bold text-slate-900 flex items-center gap-1.5">
                    <span>Google Identity Services Single Sign-On</span>
                  </h3>
                  <p className="text-xs text-slate-500 mt-0.5">
                    Sign in with your personal Gmail or university Google Workspace account as{' '}
              ...
              {/* Dynamic Live Profile Badge when user enters account */}
              {googleEmail.trim() && (
                <div className="mb-4 p-3.5 rounded-xl border border-red-100 bg-red-50/50 flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <div className="w-9 h-9 rounded-full bg-red-700 text-white font-bold flex items-center justify-center font-mono shadow-xs text-xs">
                      {(googleName.trim() || googleEmail.trim()).charAt(0).toUpperCase()}
                    </div>
                    <div>
                      <div className="flex items-center gap-1.5">
                        ...
                        <span className="text-[9px] font-mono px-1.5 py-0.2 rounded bg-emerald-100 text-emerald-800 font-semibold">
                          Google Identity
                        </span>
                      </div>
                      <p className="text-[11px] font-mono text-slate-600">{googleEmail.trim()}</p>
                    </div>
                  </div>
                  <button
                    type="button"
                    ...
                    className="text-[11px] text-slate-400 hover:text-red-700 font-mono underline cursor-pointer"
                  >
                    Clear
                  </button>
                </div>
              ...
              <form onSubmit={handleGoogleLogin} className="space-y-3.5">
                <div>
                  ...
                  <div className="relative">
                    <input
                      ...
                      placeholder="your.account@gmail.com or name@university.edu"
                      className="w-full pl-9 pr-3 py-2.5 text-xs bg-white border border-slate-200 rounded-lg focus:outline-none focus:border-red-600 transition-colors font-mono"
                    />
                ...
                <div>
                  <label className="block text-[11px] font-bold text-slate-700 uppercase tracking-wider mb-1">
                    Your Full Name <span className="text-slate-400 font-normal font-sans">(Optional)</span>
                  </label>
                  <input
                    type="text"
                    value={googleName}
                    onChange={e => setGoogleName(e.target.value)}
                    placeholder="e.g. Dr. Alex Morgan (auto-detected if left empty)"
                    className="w-full px-3 py-2 text-xs bg-white border border-slate-200 rounded-lg focus:outline-none focus:border-red-600 transition-colors"
                  />
                </div>

                {/* Quick 1-click test suggestions */}
                <div className="flex items-center gap-1.5 flex-wrap pt-0.5">
                  <span className="text-[10px] text-slate-400 font-mono">Quick test accounts:</span>
                  <button
                    type="button"
                    onClick={() => {
                      setGoogleEmail('prof.sharma@gmail.com');
                      setGoogleName('Prof. Rohit Sharma');
                    }}
                    className="text-[10px] px-2 py-0.5 rounded bg-slate-100 hover:bg-slate-200 text-slate-700 font-mono cursor-pointer transition-colors"
                  >
                    prof.sharma@gmail.com
                  </button>
                  <button
                    type="button"
                    onClick={() => {
                      setGoogleEmail('academic.lead@university.edu');
                      setGoogleName('Dr. Jane Collins');
                    }}
                    className="text-[10px] px-2 py-0.5 rounded bg-slate-100 hover:bg-slate-200 text-slate-700 font-mono cursor-pointer transition-colors"
                  >
                    academic.lead@university.edu
                  </button>
                </div>

                {/* Prominent Google Authorize Button */}
                <button
                  type="submit"
                  disabled={isLoading}
                  className="w-full mt-2 py-3 bg-red-700 hover:bg-red-800 active:bg-red-900 text-white rounded-xl text-xs font-semibold shadow-xs transition-all cursor-pointer flex items-center justify-center gap-2.5"
                >
                  {isLoading ? (
                    <>
                      <Loader2 className="w-4 h-4 animate-spin" />
                      <span>Connecting with Google Identity...</span>
                    </>
                  ) : (
                    <>
                      <svg className="w-4 h-4" viewBox="0 0 24 24">
                      ...
                      <span>
                        {googleEmail.trim()
                          ? `Sign In with Google as ${googleName.trim() || googleEmail.split('@')[0]} (${selectedRole})`
                          : `Authorize & Sign In with Google as ${selectedRole}`}
                      </span>
